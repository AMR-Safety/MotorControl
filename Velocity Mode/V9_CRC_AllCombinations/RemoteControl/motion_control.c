...
static float combinational_factor = 0.50f;    // ratio for the key combinations
...
void start_continuous_forward_left(float speed) {
	if (speed < 0) speed = current_forward_speed;
	
	current_motion_state = MOTION_FORWARD_LEFT;
	reset_motion_timeout();
	...
	if (smooth_accel_enabled) {
		target_speed_motor1 = speed*combinational_factor;
		target_speed_motor2 = speed;
		} else {
		set_angular_velocity_motor1(speed*combinational_factor);
		set_angular_velocity_motor2(speed);
	}
}

void start_continuous_forward_right(float speed) {
	if (speed < 0) speed = current_forward_speed;
	
	current_motion_state = MOTION_FORWARD_RIGHT;
	reset_motion_timeout();
	...
	if (smooth_accel_enabled) {
		target_speed_motor1 = speed;
		target_speed_motor2 = speed*combinational_factor;
		} else {
		set_angular_velocity_motor1(speed);
		set_angular_velocity_motor2(speed*combinational_factor);
	}
}

void start_continuous_backward_left(float speed) {
	if (speed < 0) speed = current_forward_speed;
	
	current_motion_state = MOTION_BACKWARD_LEFT;
	reset_motion_timeout();
...
void start_continuous_backward_right(float speed) {
	if (speed < 0) speed = current_forward_speed;
	
	current_motion_state = MOTION_BACKWARD_RIGHT;
	reset_motion_timeout();
